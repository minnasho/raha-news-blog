...
function doServerSideStuff(values, resetForm, setSubmitting) {
    const fs = require('browserify-fs');
    console.log('called');
    // write to a new file named [title].json
    const fileContent = JSON.stringify(values, null, 2);
    console.log(fileContent);
    fs.writeFile(`/posts/${fileContent.title}.json`, fileContent);
    setSubmitting(false);
    resetForm({});
...
export default function Create() {
    ...
    const doSubmit = (values, {resetForm, setSubmitting}) => {
        ...
        setSubmitting(true);
        doServerSideStuff(values, resetForm, setSubmitting);
    }
